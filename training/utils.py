...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        sample_full_episode = False  # hardcoded for now
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            image_lens = [root[f'/observations/images/{cam}'].shape[0] for cam in self.camera_names]

            # use the shortest valid sequence length
            episode_len = min(qpos_len, action_len, *image_lens)
            if episode_len == 0:
                raise ValueError(f"Episode {episode_id} has zero valid length!")
            ...
            # get single observation at start_ts
            qpos = root['/observations/qpos'][start_ts]
            image_dict = {
            ...
            # get action sequence from start_ts
            if is_sim:
                action = root['/action'][start_ts:]
                action_start = start_ts
            else:
                action = root['/action'][max(0, start_ts - 1):]  # align better with real-world timing
                action_start = max(0, start_ts - 1)

            # ensure 2D action shape: (T, A)
            if action.ndim == 1:
                action = action[:, None]

            action_len_actual = action.shape[0]
            original_action_shape = root['/action'].shape
            action_dim = action.shape[1]

            # zero-padding
            padded_action = np.zeros((original_action_shape[0], action_dim), dtype=np.float32)
            padded_action[:action_len_actual] = action

            is_pad = np.zeros(original_action_shape[0])
            is_pad[action_len_actual:] = 1
        ...
        # reorder image: [K, C, H, W] and normalize
        image_data = torch.einsum('k h w c -> k c h w', image_data)
        image_data = image_data / 255.0
...
def get_norm_stats(dataset_dir, num_episodes):
    ...
    for episode_idx in range(num_episodes):
        ...
        with h5py.File(dataset_path, 'r') as root:
            qpos = root['/observations/qpos'][()]
            action = root['/action'][()]
        all_qpos_data.append(torch.from_numpy(qpos))     # shape: [T, 8]
        all_action_data.append(torch.from_numpy(action)) # shape: [T, A]

    # 🔄 길이 다르므로 stack → cat
    all_qpos_data = torch.cat(all_qpos_data, dim=0)       # shape: [sum(T), 8]
    all_action_data = torch.cat(all_action_data, dim=0)   # shape: [sum(T), A]
    ...
    stats = {
        ...
        "example_qpos": qpos  # 마지막 qpos 그대로 리턴
    }
...
def load_data(dataset_dir, num_episodes, camera_names, batch_size_train, batch_size_val):
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size_train, shuffle=True, collate_fn=sequence_collate_fn)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size_val, shuffle=False, collate_fn=sequence_collate_fn)
