...
#set_box_position(0.3 , -0.25182, BOX_Z)
#rospy.sleep(1)
...
def capture_image(camera_name):
    camera_topics = {
        'top': '/multisense_sl/camera/left/image_raw',
        'front': '/multisense_sl/camera/right/image_raw'
    }
    topic = camera_topics.get(camera_name, '/multisense_sl/camera/left/image_raw')  # default to 'top' if not found
...
if __name__ == "__main__":
    ...
    set_box_position(box_x , box_y, BOX_Z)
    rospy.sleep(1)
    ...
    query_frequency = policy_config['num_queries'] # 149
...
# Assign the modified position list to pos
    ...
    # After loop ends, write actions to a file
            ...
            df.to_csv('actions0401.csv', index=False)  # Replace 'path_to_save_file' with your desired path
