...
class Simulator:
    def __init__(self, policy, policy_config, task_config, stats, device):
        ...
        # 카메라 초기화
        rospy.Subscriber('/camera1/color/image_raw', Image, self.left_callback)
        rospy.Subscriber('/camera2/color/image_raw', Image, self.right_callback)
        rospy.Subscriber('/camera3/color/image_raw', Image, self.gripper_callback)
    ...
    def initial_pose(self):
        initial_pose = [0, -0.7, 0, -2.35619449, 0, 1.57079632679, 0.785398163397]
        js = JointState()
    ...
    def simulate(self):
        ...
        csv_path = rospy.get_param('~csv_path', '~/Real_ACTFranka/real_dir3/csv/episode_33_action.csv')
        # Load CSV
        try:
