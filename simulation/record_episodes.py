...
class RobotTask:
    def __init__(self, camera_controller, franka):
        ...
        self.endpoint = np.array([0.44, 0.2, BOX_Z])
    ...
    def perform_task(self):
        with Progress() as progress:
            ...
            while self.success_count < TOTAL_EPISODES:
                ...
                # 2. Move to pick position and grasp it
                self.move(solution)
                self.camera_controller.gripper_width = GRASP
                self.franka.grasp_with_check(0.04, 30)
                rospy.sleep(2.0)
                ...
                self.set_box_position(self.new_x, self.new_y, BOX_Z)
                print(self.new_x, self.new_y, BOX_Z)
                self.franka.move_to_joint_position(self.initial_positions)
    ...
    def update_box_pos(self, new_x, new_y):
        z = BOX_Z
        pre_pick_pos = np.array([new_x, new_y, z + 0.05], dtype=np.float64) # z + 0.1
        pick_pos = np.array([new_x, new_y, z - 0.01], dtype=np.float64)
    ...
    def move_with_spline(self, solution, duration=5.0):
        if solution is not None:
            self.franka.move_to_joint_position_with_spline(solution.tolist(), duration)
        else :
            print('Solution does not exist')
