...
class CameraController:
    ...
    def image_callback(self, msg, camera):
        if self.recording:
            ...
            if camera == 'left':
                ...
                self.current_frame_left = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
                #self.current_frame_left = cv2.rotate(self.current_frame_left, cv2.ROTATE_90_COUNTERCLOCKWISE)#cam rotate
                self.data_left2.append(self.current_frame_left)
            elif camera == 'right':
                self.current_frame_right = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
                #self.current_frame_right = cv2.rotate(self.current_frame_right, cv2.ROTATE_90_CLOCKWISE)#cam rotate
                self.data_right2.append(self.current_frame_right)
            elif camera == 'gripper':
                self.current_frame_gripper = self.bridge.imgmsg_to_cv2(msg, desired_encoding="bgr8")
                #self.current_frame_gripper = cv2.rotate(self.current_frame_gripper, cv2.ROTATE_180) #gripper cam rotate
    ...
    def save_data(self):
        ...
        csv_path = os.path.join(directory2, f'episode_{episode_idx}_action.csv')
